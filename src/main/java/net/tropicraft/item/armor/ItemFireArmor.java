...
public class ItemFireArmor extends ItemTropicraftArmor
{
    ...
    public void onArmorTick(final World world, final EntityPlayer player, final ItemStack itemStack) {
        if (world.isRemote) {
            ...
            if (world.getWorldTime() % factor == 0L && world.canBlockSeeTheSky(MathHelper.floor_double(player.posX), MathHelper.floor_double(player.posY + 1.0), MathHelper.floor_double(player.posZ))) {
    ...
    public static void getFXLayers() {
        ...
        try {
            ...
            ItemFireArmor.fxLayers = (List[])field.get(FMLClientHandler.instance().getClient().effectRenderer);
        }
        catch (Exception ex3) {
            try {
                field = EffectRenderer.class.getDeclaredField("fxLayers");
    ...
    public void clientTick(final EntityPlayer player) {
        ...
        if (this == TCItemRegistry.fireBoots) {
            ...
            if (player.motionY < 0.0) {
                final Block block = player.worldObj.getBlock(MathHelper.floor_double(player.posX + x), MathHelper.floor_double(player.posY - 2.0), MathHelper.floor_double(player.posZ + z));
                if (block != null && block.getMaterial() == Material.lava) {
                    onLava = true;
                }
            }
            final Block block2 = player.worldObj.getBlock(MathHelper.floor_double(player.posX + x), MathHelper.floor_double(player.posY - 1.0), MathHelper.floor_double(player.posZ + z));
            ...
            for (int i = 0; i < 1 + (onLava ? 5 : 0); ++i) {
                ...
                if (onLava || rand.nextInt(1 + extraRand) == 0) {
                    player.worldObj.spawnParticle(particle, gatherX + (rand.nextFloat() * range - range / 2.0f), gatherY + (rand.nextFloat() * range - range / 2.0f), gatherZ + (rand.nextFloat() * range - range / 2.0f), player.motionX + motionX, 0.009999999776482582, player.motionZ + motionZ);
                    player.worldObj.spawnParticle(particle, gatherX + (rand.nextFloat() * range - range / 2.0f), gatherY + (rand.nextFloat() * range - range / 2.0f), gatherZ + (rand.nextFloat() * range - range / 2.0f), player.motionX - motionX, 0.009999999776482582, player.motionZ - motionZ);
                }
            }
        }
            ...
            if (rand.nextInt(3 + extraRand) == 0) {
                player.worldObj.spawnParticle(particle2, player.posX + (rand.nextFloat() * range - range / 2.0f), player.posY - 0.800000011920929 + (rand.nextFloat() * range - range / 2.0f), player.posZ + (rand.nextFloat() * range - range / 2.0f), (double)(rand.nextFloat() * speed - speed / 2.0f), -0.05000000074505806, (double)(rand.nextFloat() * speed - speed / 2.0f));
            }
        }
        else if (this == TCItemRegistry.fireChestplate) {
            final float look2 = -180.0f;
            final double dist2 = 0.5;
            final double gatherX2 = player.posX + -Math.sin((player.rotationYaw + look2) / 180.0f * 3.1415927f) * Math.cos(player.rotationPitch / 180.0f * 3.1415927f) * dist2;
            final double gatherZ2 = player.posZ + Math.cos((player.rotationYaw + look2) / 180.0f * 3.1415927f) * Math.cos(player.rotationPitch / 180.0f * 3.1415927f) * dist2;
            String particle3 = "flame";
            if (rand.nextInt(2) == 0) {
                particle3 = "smoke";
            }
            if (rand.nextInt(3 + extraRand) == 0) {
                player.worldObj.spawnParticle(particle3, gatherX2 + (rand.nextFloat() * range - range / 2.0f), player.posY - 0.4000000059604645 + (rand.nextFloat() * range - range / 2.0f), gatherZ2 + (rand.nextFloat() * range - range / 2.0f), (double)(rand.nextFloat() * speed - speed / 2.0f), -0.009999999776482582, (double)(rand.nextFloat() * speed - speed / 2.0f));
            }
        }
            ...
            if (rand.nextInt(2) == 0) {
                player.worldObj.spawnParticle(particle3, gatherX2 + (rand.nextFloat() * range - range / 2.0f), player.posY + 0.699999988079071, gatherZ2 + (rand.nextFloat() * range - range / 2.0f), (double)(rand.nextFloat() * speed - speed / 2.0f), -0.009999999776482582, (double)(rand.nextFloat() * speed - speed / 2.0f));
            }
            if (ItemFireArmor.fxLayers != null) {
    ...
    public void moveEnt(final Entity ent, final Entity center, final boolean shield) {
        ...
        final double gatherX = center.posX + -Math.sin((center.rotationYaw + look) / 180.0f * 3.1415927f) * Math.cos(center.rotationPitch / 180.0f * 3.1415927f) * dist;
        final double gatherY = center.posY + 0.8 + -MathHelper.sin(center.rotationPitch / 180.0f * 3.1415927f) * dist - 0.0;
        final double gatherZ = center.posZ + Math.cos((center.rotationYaw + look) / 180.0f * 3.1415927f) * Math.cos(center.rotationPitch / 180.0f * 3.1415927f) * dist;
        ...
        if (entDist > -1.0) {
            ...
            float angle;
            float f;
            for (f = (angle = (float)(Math.atan2(vecZ, vecX) * 180.0 / 3.1415927410125732)), angle += 180.0f, angle = f; angle < 0.0f; angle += 360.0f) {}
            while (angle >= 360.0f) {
                angle -= 360.0f;
            }
