...
public class ItemDiveComputer extends ItemMap
{
    ...
    public void onUpdate(final ItemStack itemstack, final World world, final Entity entity, final int par4, final boolean par5) {
        ...
        if (this.ticksUntilUpdate <= 0) {
            ...
            player.setAir(300);
            final float air = this.getTagCompound(chestplateStack).getFloat("AirContained");
            final ItemScubaGear.AirType airType = (chestplateStack.getItemDamage() >= 2) ? ItemScubaGear.AirType.TRIMIX : ItemScubaGear.AirType.REGULAR;
            ...
            int y;
            int z;
            for (x = MathHelper.floor_double(player.posX), y = MathHelper.floor_double(player.posY + player.height - 0.5), z = MathHelper.floor_double(player.posZ); world.getBlock(x, y + waterBlocksAbove + 1, z).getMaterial().isLiquid(); ++waterBlocksAbove) {}
    ...
    private boolean armorCheck(final World world, final EntityPlayer player, final ItemStack helmetStack, final ItemStack chestplateStack, final ItemStack leggingsStack, final ItemStack flippersStack) {
