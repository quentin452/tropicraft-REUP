...
public class ItemSnareTrap extends ItemTropicraft
{
    ...
    public boolean onItemUse(final ItemStack stack, final EntityPlayer player, final World world, int x, final int y, int z, final int side, final float offsetX, final float offsetY, final float offsetZ) {
        ...
        final List objs = world.getEntitiesWithinAABB((Class)EntitySnareTrap.class, AxisAlignedBB.getBoundingBox((double)x, (double)(y - height + 1), (double)z, (double)(x + 1), (double)(y - height + 1 + 6), (double)(z + 1)));
        if (!objs.isEmpty()) {
            return false;
        }
