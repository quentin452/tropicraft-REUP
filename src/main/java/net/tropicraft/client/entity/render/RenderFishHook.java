...
public class RenderFishHook extends Render
{
    ...
    public void doRenderFish(final EntityTropicalFishHook entityfish, final double d, final double d1, final double d2, final float f, final float f1) {
        GL11.glPushMatrix();
        ...
        if (entityfish.angler != null) {
            final float f9 = (entityfish.angler.prevRotationYaw + (entityfish.angler.rotationYaw - entityfish.angler.prevRotationYaw) * f1) * 3.141593f / 180.0f;
            ...
            double d5 = entityfish.angler.prevPosX + (entityfish.angler.posX - entityfish.angler.prevPosX) * f1 + vec3d.xCoord;
            double d6 = entityfish.angler.prevPosY + (entityfish.angler.posY - entityfish.angler.prevPosY) * f1 + vec3d.yCoord;
            double d7 = entityfish.angler.prevPosZ + (entityfish.angler.posZ - entityfish.angler.prevPosZ) * f1 + vec3d.zCoord;
            if (this.renderManager.options.thirdPersonView <= 0) {}
            final float f12 = (entityfish.angler.prevRenderYawOffset + (entityfish.angler.renderYawOffset - entityfish.angler.prevRenderYawOffset) * f1) * 3.141593f / 180.0f;
            final double d8 = MathHelper.sin(f12);
            final double d9 = MathHelper.cos(f12);
            d5 = entityfish.angler.prevPosX + (entityfish.angler.posX - entityfish.angler.prevPosX) * f1 - d9 * 0.35 - d8 * 0.85;
            d6 = entityfish.angler.prevPosY + (entityfish.angler.posY - entityfish.angler.prevPosY) * f1 - 0.45;
            d7 = entityfish.angler.prevPosZ + (entityfish.angler.posZ - entityfish.angler.prevPosZ) * f1 - d8 * 0.35 + d9 * 0.85;
