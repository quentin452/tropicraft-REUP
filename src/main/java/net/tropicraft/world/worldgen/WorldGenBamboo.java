...
public class WorldGenBamboo extends TCGenBase
{
    ...
    public boolean generate(final int i, int j, final int k) {
        ...
        if (this.worldObj.getBlock(i + 1, j - 1, k).getMaterial() != Material.water && this.worldObj.getBlock(i - 1, j - 1, k).getMaterial() != Material.water && this.worldObj.getBlock(i, j - 1, k + 1).getMaterial() != Material.water && this.worldObj.getBlock(i, j - 1, k - 1).getMaterial() != Material.water) {
            return false;
        }
        final int amount = this.rand.nextInt(30) + 30;
        final int spread = this.rand.nextInt(3) - 1 + (int)(Math.sqrt(amount) / 2.0);
        for (int l = 0; l < amount; ++l) {
            for (int x = i + this.rand.nextInt(spread) - this.rand.nextInt(spread), z = k + this.rand.nextInt(spread) - this.rand.nextInt(spread), y = this.getTerrainHeightAt(x, z), height = this.rand.nextInt(4) + 4, h = 0; h < height && this.worldObj.isAirBlock(x, y + h, z); ++h) {
