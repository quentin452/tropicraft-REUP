...
public class TileEntityAirCompressor extends TileEntity
{
    ...
    public void readFromNBT(final NBTTagCompound nbt) {
        ...
        if (nbt.hasKey("Tank")) {
            ...
            this.maxCapacity = ((this.tank.getItemDamage() == 1) ? ((float)ItemScubaGear.AirType.TRIMIX.getMaxCapacity()) : ((float)ItemScubaGear.AirType.REGULAR.getMaxCapacity()));
        }
        else {
            this.tank = null;
        }
    ...
    public void updateEntity() {
        ...
        final float airContained = this.getTagCompound(this.tank).getFloat("AirContained");
        if (this.compressing && airContained < ItemScubaGear.AirType.REGULAR.getMaxCapacity()) {
            if (airContained + 0.1f >= ItemScubaGear.AirType.REGULAR.getMaxCapacity()) {
                this.tank.getTagCompound().setFloat("AirContained", (float)ItemScubaGear.AirType.REGULAR.getMaxCapacity());
                ++this.ticks;
                this.finishCompressing();
            }
            else {
                this.tank.getTagCompound().setFloat("AirContained", airContained + 0.1f);
                ++this.ticks;
            }
        }
    }
    ...
    public void ejectTank() {
        if (this.tank != null) {
            final EntityItem tankItem = new EntityItem(this.worldObj, (double)this.xCoord, (double)this.yCoord, (double)this.zCoord, this.tank);
