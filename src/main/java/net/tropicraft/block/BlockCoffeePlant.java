...
public class BlockCoffeePlant extends BlockTropicraft
{
    ...
    public boolean shouldSideBeRendered(final IBlockAccess world, final int x, final int y, final int z, final int side) {
        return (side != 1 || world.getBlock(x, y, z) != this) && (side != 0 || world.getBlock(x, y, z) != this) && super.shouldSideBeRendered(world, x, y, z, side);
    }
    ...
    public boolean canBlockStay(final World world, final int x, final int y, final int z) {
        ...
        return (world.getFullBlockLightValue(x, y, z) >= 8 || world.canBlockSeeTheSky(x, y, z)) && soil != null && (soil == this || soil.canSustainPlant((IBlockAccess)world, x, y - 1, z, ForgeDirection.UP, (IPlantable)TCItemRegistry.coffeeBean));
    }
    ...
    public void registerBlockIcons(final IIconRegister iconRegister) {
        ...
        for (int i = 0; i < 7; ++i) {
            this.leafIcons[i] = iconRegister.registerIcon(this.getUnlocalizedName().substring(this.getUnlocalizedName().indexOf(".") + 1) + "_" + i);
        }
        this.stemIcon = iconRegister.registerIcon(this.getUnlocalizedName().substring(this.getUnlocalizedName().indexOf(".") + 1) + "_" + "Stem");
    }
}
