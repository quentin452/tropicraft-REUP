...
public class BlockTropicraftSlab extends BlockSlab
{
    ...
    public float getPlayerRelativeBlockHardness(final EntityPlayer par1EntityPlayer, final World par2World, final int j, final int k, final int l) {
        final int type = par2World.getBlockMetadata(j, k, l) & 0x7;
        if (type < BlockTropicraftSlab.modelBlocks.length) {
            return BlockTropicraftSlab.modelBlocks[type].getPlayerRelativeBlockHardness(par1EntityPlayer, par2World, j, k, l);
        }
    ...
    static {
        BlockTropicraftSlab.modelBlocks = new Block[] { (Block)TCBlockRegistry.bambooBundle, (Block)TCBlockRegistry.thatchBundle, (Block)TCBlockRegistry.chunkOHead, (Block)TCBlockRegistry.palmStairs };
    }
}
