...
public class EntitySnareTrap extends Entity implements IEntityAdditionalSpawnData
{
    ...
    public EntitySnareTrap(final World world, final int x, final int y, final int z, final int height, final ForgeDirection dir) {
        this(world);
        this.trapPosX = x;
        this.trapPosY = y;
        this.trapPosZ = z;
    ...
    public Block getHookBlock(final World world) {
        return world.getBlock(this.trapPosX - this.trapDirection.offsetX, this.trapPosY + this.trapHeight - 1, this.trapPosZ - this.trapDirection.offsetZ);
    }
    ...
    public void onUpdate() {
        ...
        if (this.worldObj.isRemote) {
            ...
            if (this.getUsername().equals(player.getGameProfile().getName())) {
                player.setLocationAndAngles(this.trapPosX + 0.5, (double)this.trapPosY, this.trapPosZ + 0.5, player.rotationYaw, player.rotationPitch);
            }
            return;
        }
        if (this.caughtEntity == null) {
            ...
            if (this.scanCoolDown == 0) {
                this.scanCoolDown = 5;
                final List ents = this.worldObj.getEntitiesWithinAABBExcludingEntity((Entity)this, this.boundingBox.expand(5.0, 5.0, 5.0));
                for (final Object obj : ents) {
                    final Entity ent = (Entity)obj;
                    if (ent instanceof EntityLivingBase && ((EntityLivingBase)ent).isEntityAlive() && ent.posX >= this.trapPosX && ent.posX <= this.trapPosX + 1 && ent.posZ >= this.trapPosZ && ent.posZ <= this.trapPosZ + 1 && Math.abs(this.trapPosY - ent.posY) < 0.01) {
            ...
            this.caughtEntity.setLocationAndAngles(this.trapPosX + 0.5, (double)this.trapPosY, this.trapPosZ + 0.5, this.caughtEntity.rotationYaw, this.caughtEntity.rotationPitch);
            this.caughtEntity.motionX = 0.0;
            this.caughtEntity.motionY = 0.0;
            this.caughtEntity.motionZ = 0.0;
        }
        if (this.worldObj.getBlock(this.trapPosX - this.trapDirection.offsetX, this.trapPosY + this.trapHeight - 1, this.trapPosZ - this.trapDirection.offsetZ) != this.getHookBlock(this.worldObj) || this.worldObj.getBlock(this.trapPosX, this.trapPosY - 1, this.trapPosZ) != this.getBottomBlock(this.worldObj)) {
            if (this.caughtEntity != null) {
                this.releaseEntity();
            }
    ...
    public void catchEntity(final EntityLivingBase ent) {
        ...
        this.worldObj.playSoundAtEntity((Entity)this, "random.bow", 1.0f, 1.0f / (this.rand.nextFloat() * 0.4f + 1.2f));
        this.setSize(0.6f, this.trapHeight - ent.height - ent.yOffset - 0.5f);
        this.setLocationAndAngles(this.trapPosX + 0.5, (double)(this.trapPosY + ent.height + ent.yOffset), this.trapPosZ + 0.5, 0.0f, 0.0f);
    ...
    public void releaseEntity() {
        ...
        if (this.caughtEntity instanceof EntityPlayer) {
            ...
            Tropicraft.eventChannel.sendTo(PacketHelper.getNBTPacket(nbt, Tropicraft.eventChannelName), (EntityPlayerMP)this.caughtEntity);
        }
        else {
