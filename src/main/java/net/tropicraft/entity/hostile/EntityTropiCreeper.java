        ...
        for (int x = (int)this.posX - xo; x < (int)this.posX + xo; ++x) {
            for (int z = (int)this.posZ - zo; z < (int)this.posZ + zo; ++z) {
                if (this.rand.nextInt(3) != 0) {
                    for (y = (int)this.posY + 3; y > (int)this.posY - 3 && (!this.worldObj.isAirBlock(x, y, z) || !this.worldObj.getBlock(x, y - 1, z).isOpaqueCube()); --y) {}
                    if (y != (int)this.posY - 3) {
                        if (this.rand.nextInt(10) < 6 && this.worldObj.getBlock(x, y, z) != TCBlockRegistry.bambooBundle && this.worldObj.getBlock(x, y, z) != TCBlockRegistry.coconut) {
                            ...
                            if (TCBlockRegistry.flowers.canPlaceBlockAt(this.worldObj, x, y, z)) {
                                ...
                                this.entityDropItem(new ItemStack((Block)TCBlockRegistry.flowers, 1, flowerType), 0.5f);
                            }
                        }
                        else if (this.rand.nextInt(10) < 7 && !CoroUtilBlock.isEqual(this.worldObj.getBlock(x, y, z), (Block)TCBlockRegistry.bambooBundle) && !CoroUtilBlock.isEqual(this.worldObj.getBlock(x, y, z), (Block)TCBlockRegistry.coconut) && CoroUtilBlock.isEqual(this.worldObj.getBlock(x, y - 1, z), (Block)Blocks.grass)) {
                            this.worldObj.setBlock(x, y, z, (Block)Blocks.tallgrass, 1, 3);
                        }
                        else if (this.rand.nextInt(10) < 8 && this.worldObj.getBlock(x, y - 1, z) != TCBlockRegistry.bambooBundle && !CoroUtilBlock.isAir(this.worldObj.getBlock(x, y - 1, z))) {
