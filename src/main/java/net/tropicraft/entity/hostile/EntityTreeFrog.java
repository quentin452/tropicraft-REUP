...
public abstract class EntityTreeFrog extends EntityTropicraftAnimal implements IEntityAdditionalSpawnData, IRangedAttackMob
{
                ...
                if (this.jumpDelay <= 0) {
                    ...
                    Label_0237: {
                        if (this.getNavigator().getPath() != null) {
                            try {
                                if (this.getNavigator().getPath().getCurrentPathIndex() > this.getNavigator().getPath().getCurrentPathLength()) {
                                    this.getNavigator().clearPathEntity();
                                    return;
                                }
                                final Vec3 pos = this.getNavigator().getPath().getPosition((Entity)this);
    ...
    public void attackEntityWithRangedAttack(final EntityLivingBase entity, final float f) {
        ...
        if (f < 4.0f && !this.worldObj.isRemote && this.attackTime == 0 && this.worldObj.difficultySetting != EnumDifficulty.PEACEFUL) {
            final double d = entity.posX - this.posX;
            final double d2 = entity.posZ - this.posZ;
            final EntityPoisonBlot entityPoisonBlot;
            final EntityPoisonBlot entitypoisonblot = entityPoisonBlot = new EntityPoisonBlot(this.worldObj, (EntityLivingBase)this);
            ...
            this.worldObj.playSoundAtEntity((Entity)this, "frogspit", 1.0f, 1.0f / (this.rand.nextFloat() * 0.4f + 0.8f));
