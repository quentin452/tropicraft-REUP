...
public class EntityTropicraftLeafballNew extends EntityThrowableUsefull
{
    ...
    protected void onImpact(final MovingObjectPosition movingobjectposition) {
        if (movingobjectposition.entityHit != null && !this.worldObj.isRemote) {
            final byte byte0 = 2;
            if (movingobjectposition.entityHit instanceof ICoroAI && (this.getThrower() == null || this.getThrower() instanceof ICoroAI)) {
                if (this.getThrower() != null && ((ICoroAI)this.getThrower()).getAIAgent().dipl_info.isEnemy(((ICoroAI)movingobjectposition.entityHit).getAIAgent().dipl_info)) {
